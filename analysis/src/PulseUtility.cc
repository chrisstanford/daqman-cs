...
bool FirstAmplitudeIsSmaller(double x1, double x2, double dumb_sign, double precision){
  if(dumb_sign>0) return (x2-x1>std::fabs(precision));
  else if(dumb_sign<0) return (x1-x2>std::fabs(precision));
  else {
...
//calculate the running sum waveform
